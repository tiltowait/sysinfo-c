...
static void beastie_init(BeastiePrinter *bp) {
    bp->index = 0;
}
...
static void beastie_print_line(BeastiePrinter *bp, const char *label, const char *value) {
    printf("%s%s%s\t%s%s:%s\t%s\n",
           RED_BOLD, beastie_next(bp), RESET,
           RED_BOLD, label, RESET,
           value);
}
...
static int get_memory(char *buf, size_t bufsize) {
    ...
    snprintf(buf, bufsize, "%s / %.0fG (%s Avail)",
             used_str, bytes_to_gb(physmem), avail_str);
    return 0;
}
...
static int get_arc(char *buf, size_t bufsize) {
    ...
    snprintf(buf, bufsize, "%s Total, %s MFU, %s MRU (%.1f%%)",
             arc_str, mfu_str, mru_str, cache_pct);
    return 0;
}
...
static int get_interface_addrs(const char *ifname, InterfaceAddr *addrs, int max_addrs) {
